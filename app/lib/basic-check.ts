...
export function basicCheckHtml(url: string, html: string, extraHtml?: string): CheckResult {
  ...
  function add(id: string, evidence: string) {
  ...
  if (gaMatch) {
    add("google-analytics", `Найден код Google Analytics (${gaMatch[0].slice(0, 60)}). Данные посетителей уходят в США автоматически. Если GA не упомянут в вашей политике конфиденциальности — это двойное нарушение: и локализации, и трансграничной передачи. Если упомянут — риск по трансграничной передаче снижается, но нарушение локализации остаётся.`);
  }

  // 2. Google Fonts
  const gfMatch = html.match(/fonts\.googleapis\.com|fonts\.gstatic\.com/i);
  if (gfMatch) {
    add("google-fonts", `Найдена ссылка на Google Fonts: «${gfMatch[0]}»`);
  }

  // 3. Meta Ads Pixel
  const fbPixelMatch = html.match(/connect\.facebook\.net|fbq\s*\(/i);
  if (fbPixelMatch) {
    add("meta-ads-pixel", "Найден код Facebook/Meta Pixel — индикатор рекламной активности в запрещённой соцсети");
  }

  // 4. Meta Logo — логотипы/иконки Instagram/Facebook
  ...
  if (metaLogoMatch) {
    add("meta-logo", `Найдена символика Meta (логотип/иконка): «${metaLogoMatch[0].slice(0, 80)}»`);
  }

  // 5. Текстовые упоминания Instagram/Facebook без пометки об экстремизме
  ...
  if (metaTextMatch) {
    ...
    if (!isFilePath) {
      ...
      if (!hasDisclaimer) {
        add("meta-links-text", `Упоминание Meta-сервиса без пометки об экстремизме: «${textContext.trim().slice(0, 80)}»`);
      }
    }
  }
  ...
  if (!privacyLink) {
    add("pd-policy", "На странице не найдено ссылок на политику конфиденциальности");
  }

  // 7. Cookie-баннер
  const hasCookieWord = /cookie|куки/i.test(html);
  if (!hasCookieWord) {
    add("cookie-banner", "Не найден баннер согласия на cookies");
  } else {
    ...
    if (!hasConsent) {
      add("cookie-banner", "Слово «cookie» найдено, но рядом нет текста о согласии или информировании");
    }
  }

  // 8. SSL
  if (!url.startsWith("https://")) {
    add("ssl", "Сайт не использует HTTPS");
  }
  ...
  if (navText.length > 20) {
    ...
    if (letterCount > 10 && cyrillicCount / letterCount < 0.2) {
      ...
      add("lang-headers", `В навигации преобладают слова на иностранном языке: «${sample}»`);
    }
  }
  ...
  if (foundAnglicisms.length >= 2) {
    add("lang-anglicisms", `Найдены англицизмы: ${foundAnglicisms.join(", ")}`);
  }

  // 11. Реквизиты юр.лица (ИНН + ОГРН + название) — проверяем во всех доступных страницах
  ...
  if (!hasInn || !hasOgrn || !hasLegalName) {
    ...
    if (extraHtml) {
      add("requisites-missing", `Не найдено ни на главной, ни на странице контактов: ${missing}`);
    } else {
      add("requisites-missing", `Не найдено на главной странице: ${missing}. Реквизиты могут находиться на странице «Контакты» — чекер не смог найти её автоматически. Проверьте вручную или запустите Pro-режим.`);
    }
  }

  // 12. Контакты (телефон или email)
  const phoneRegex = /(?:\+7|8)[\s\-()]?\d{3}[\s\-()]?\d{3}[\s\-()]?\d{2}[\s\-()]?\d{2}/;
  const emailRegex = /[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}/i;
  if (!phoneRegex.test(fullHtml) && !emailRegex.test(fullHtml)) {
    add("contacts-missing", "Не найден ни телефон, ни email на главной странице");
  }

  // 13. Трансграничная передача ПДн (если есть иностранные сервисы)
  ...
  // 14. ЗоЗПП-проверки (только для коммерческих сайтов)
  ...
  if (commercial.isCommercial) {
    ...
    if (foreignCurrencyRegex.test(html) && !rubPriceRegex.test(html)) {
      add("prices-currency", "Найдены цены в иностранной валюте без рублёвого эквивалента");
    }

    if (!/<a[^>]*>[^<]*(?:оферт|условия использования|пользовательское соглашение|публичный договор)/i.test(html)) {
      add("offer-missing", "На сайте не найдена ссылка на оферту или условия продажи");
    }
    ...
    if (hasEcommerceSignal) {
      if (!/<a[^>]*>[^<]*(?:возврат|обмен|гарантия товар)/i.test(html)) {
        add("return-policy-missing", "Не найдена страница с условиями возврата товара");
      }
    }
  }
